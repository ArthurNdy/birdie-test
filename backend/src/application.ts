...
app.get("/daily", async(_,res) => {
  const data = await sequelize.query("SELECT id, timestamp, event_type, alert_id, care_recipient_id, payload_as_text FROM birdietest.events WHERE DATE_FORMAT(timestamp, '%d/%m/%Y') = '24/04/2019' AND care_recipient_id = 'e3e2bff8-d318-4760-beea-841a75f00227' ORDER BY TIMESTAMP(timestamp)")
  console.log('data', data[0]);
  res.send(data[0]);
});

app.get("/alerts", async(_,res) => {
  const data = await sequelize.query("SELECT id, timestamp, event_type, alert_id, care_recipient_id, payload_as_text FROM birdietest.events WHERE DATE_FORMAT(timestamp, '%d/%m/%Y') = '24/04/2019' AND care_recipient_id = 'e3e2bff8-d318-4760-beea-841a75f00227' AND alert_id IS NOT NULL ORDER BY TIMESTAMP(timestamp)")
  console.log('data', data[0]);
  res.send(data[0]);
});
